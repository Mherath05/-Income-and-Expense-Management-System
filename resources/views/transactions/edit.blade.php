...
<form action="{{ route('transactions.update', $transaction) }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="number" id="amount" name="amount" step="0.01" min="0.01" value="{{ old('amount', $transaction->amount) }}" required>
        @error('amount')
            <div style="color: red; font-size: 14px;">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description" name="description" rows="3">{{ old('description', $transaction->description) }}</textarea>
        @error('description')
            <div style="color: red; font-size: 14px;">{{ $message }}</div>
        @enderror
    </div>
